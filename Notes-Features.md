...
Selected images are copied, orientation-corrected and compressed into encrypted note content; they appear after the existing content, with tap-to-preview/removal. Each image has an editable paragraph beneath it; the last paragraph provides at least 120 dp of tappable writing space. Text can remain between multiple images, and image positions survive editing, reopening and backup. The first image build uses bounded previews up to 1440 pixels per side and 250 KB JPEG per image, within the existing note-size budget. Source removal does not affect the saved image. Images and formatting participate in autosave, undo/redo, Bin retention and app-wide backup/restore. Camera capture and templates are outside this requested increment.
