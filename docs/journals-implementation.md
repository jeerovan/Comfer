...
Updated 19 September 2026. This record supplements Plan-Journals.md; incomplete acceptance checks remain open there.
...
- `journals.db` is under `noBackupFilesDir`, Room schema 5. Journals is pre-production, so earlier development schemas and plaintext storage have no migration path. Restore generation rejects queued writes after replacement; indexed date/order metadata supports bounded queries. The feed retains at most 150 entries plus one lookahead row; Trash retains 100 plus one, with explicit older/newer paging.
...
- **Protect Journals** encrypts entries, drafts, segments and images with AES-256-GCM. Protected content uses a Journal-specific key derived from the shared authenticated session root; the root is persisted only as device-authentication-protected Keystore ciphertext. It stays in memory while a Notes/Journal screen is open and for the selected time-away interval, then is cleared. Unprotected content keeps its local envelope encryption. Both formats keep large payloads out of Keystore operations. IDs, dates, revisions and reference/order columns remain plaintext metadata; search decrypts in memory. Protection changes transactionally re-encrypt existing content. Prior protected cipher payloads upgrade after native authentication, preserving content, revisions and image IDs. Missing or damaged keys fail closed without resetting data; portable backup recovery remains available on a fresh installation.
- Protected manual backup uses the shared verification session (requesting credentials if expired), then encrypts the Journal section (including images) with AES-256-GCM, random salt/nonce and password-derived PBKDF2-HMAC-SHA1 (1,300,000 iterations, API-24 compatible). Export password is portable, not the device credential; forgotten passwords cannot be recovered. One shared backup password encrypts both Notes and Journals whenever either is protected; other Comfer archive sections keep their existing formats. Journal archive schema 1 remains portable and backward compatible. Same-device restore checkpoints now contain ciphertext, and cold recovery replays it without authentication. Restoring into an already-protected Journal keeps protection and rekeys staged images to its protected key. Older missing sections preserve current Journals; present sections replace current Journals, including empty data. Restore confirmation shows only the source Comfer version and localized backup date/time, with password input for encrypted archives.
...
16 September Journal search: added a gray leading composer search button, Search Journal mode, separate saved query state, 300 ms debounce and Room Flow text search across all non-archived dates, using the existing bounded 50–150-entry window and older/newer paging. Empty queries show a prompt; SQL substring matching treats `%` and `_` literally. SQLite lower-case matching folds ASCII case; non-ASCII text remains literal. Search reuses the normal entry cards and mutation paths, hides date navigation/photo/mic/submit controls, and preserves the composer draft. Close or Back exits search after resolving any active edit. No database schema change or migration is needed.

Validation: 14 distinct Journal UI/database cases passed across the final 13-pass suite and targeted theme rerun, including three new search cases for debounce/query replacement/clearing, protected draft preservation, cross-date text editing, date/time picker save, swipe delete/Undo, archive exclusion, literal symbols and pagination. Existing image preview/zoom/pan/removal, RTL/large text, themes and normal feed behavior passed. Search image interactions reuse that same implementation but were not separately exercised in search mode. Initial checks exposed the generic test host's adjust-pan keyboard behavior (unlike JournalActivity's declared adjustResize); matching the host setting fixed header visibility. The existing theme pixel check also needed to await the emulator compositor's displayed frame. All 167 JVM tests and the debug build passed. Emulator updated; Samsung not updated. Reports: `validation-artifacts/device-checkpoint/2026-09-16-journal-search/`.
...
## 19 September 2026: Protect Journals

Validation on the Android 7 Small_Phone emulator: 57 instrumentation checks passed (7 encryption, 4 protected backup/recovery, 13 persistence/migration, 12 existing backup, 15 screen UI and 6 activity tests). Tests cover the 2 MB media boundary, real Keystore envelope round-trips, ciphertext substitution/tampering, transaction rollback, legacy text/photo migration and logical plaintext cleanup, password rejection and full portable restore, locked checkpoint recovery, and the real system credential prompt/cancellation. Protection-mode success/failure injection uses a test cipher; successful PIN entry and authenticated private-key use still require manual validation. The debug build passed; signed release was not rebuilt for this change.

19 September clarification: removed pre-production Journal schema/plaintext/checkpoint migration paths and the unused earlier cipher reader. Password dialogs now explicitly describe one shared Notes/Journal backup password, independent of device credentials. Existing development databases without encrypted storage are unsupported; no destructive fallback is enabled.

Validation after this clarification: debug build and 25 focused API-24 instrumentation tests passed (7 encryption, 5 protected backup/recovery, 12 persistence, 1 password dialog). Coverage includes shared-password encryption of both archive sections, successful full restore, wrong-password rejection, protection rekey rollback, unsupported storage/checkpoint rejection, and updated dialog validation. Successful real PIN authentication remains a manual check; signed release was not rebuilt.

19 September protection timeout: Journal settings now show only the Protect Journals switch. Global Settings → Protection Timeout shows “For notes and journals.” and 30 sec / 2 min / 5 min, defaulting to 2 min. Both modules and protected backup/restore reuse the same successful device verification. The timeout counts only after leaving the last Notes/Journal screen; returning before expiry keeps the session, and the next exit starts a fresh interval. App process death clears it. The preference is persisted in the existing global settings store, while verification stays only in memory.

Protection timeout validation: debug build, 7 deterministic timing tests and 37 API-24 emulator tests passed (2 shared-session lifecycle, 2 setting UI/persistence, 6 Journal activity, 7 Journal encryption, 16 Notes storage, 4 Notes save-cancellation). Navigation, recreation, reopening, credential prompt/cancellation, exact expiry boundaries, preference changes, and persisted timeout selection were exercised. Journal private lifecycle fixtures use a software test cipher and Notes uses an empty protected module; successful PIN entry with native protected keys remains a manual validation step. Signed release was not rebuilt.

19 September time-away correction: foreground verification no longer expires. Activity owners keep the shared session open; only the last owner leaving starts the configured countdown. Returning after expiry does not revive authorization. Foreground cryptography now uses AES-GCM keys derived per module/generation from a session root, whose only persisted copy is wrapped by a device-authentication-required Keystore key. Expiry clears the root cache on a scheduled callback and on any access. After native authentication, both modules upgrade previous authenticated-key ciphertext while the device token is fresh. Note and draft IDs/revisions remain unchanged; Journal images are atomically re-encrypted in place, retaining their IDs and checkpoint compatibility. Portable backup formats/password behavior are unchanged.

Time-away validation: final debug build, 11 deterministic timer tests and 47 API-24 instrumentation tests passed (3 shared lifecycle including a real 31-second foreground save followed by 31 seconds away, 3 session-key crypto/failure tests, 17 Notes storage/upgrade, 9 Journal encryption/upgrade, 5 protected backup/recovery, 6 Journal activity, 4 Notes save cancellation). Repeated recreation and native prompt cancellation passed. The PIN keyboard required a second Back in the test helper; Notes resume checks now run after lifecycle resume dispatch. Crypto tests use a controlled authenticated wrapper/software fixtures for positive private-key paths; successful native PIN-to-key unwrap and existing-data upgrade still require manual device validation. No signed release build was produced.
