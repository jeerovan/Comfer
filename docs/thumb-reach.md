...
- A downward drag first scrolls existing content toward its beginning. Only movement left over at the top boundary reveals space above the content, lowering the scrollable content toward the thumb. Notification Inbox and Journal keep their screen titles outside this scrolling region.
...
- Keep bottom navigation/action controls fixed. Notification Inbox also keeps its module title and current-page heading fixed at the top across Inbox, Saved history and settings subpages; calculate reach space from the body viewport between the header and bottom controls. Content can use the full viewport while scrolling; do not resize the screen to a 360 dp viewport.
- Landscape and safe heights at or below 360 dp need no additional reach space and clear the offset. Toolbar resizing retains the user's existing offset, updating only the limit for further pulling; selection/deselection must not reset content to the top. Newly opened Inbox subpages start without reach space; existing content scroll-position rules remain separate.
