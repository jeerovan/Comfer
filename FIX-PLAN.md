...
- [ ] **P5-01** Status: IN PROGRESS — Unit tests, lint, release/benchmark compilation, API 24/API 36 and physical API 30 app instrumentation pass. Version 39 -> 41 migration fixtures now cover every legacy store, exact storage names/delimiter, empty lists, malformed folder/widget aborts, retry ordering, and source preservation on failure. Version 40 -> 41 coverage proves the unchanged DataStore/Room schema contract and full legacy bypass. Disposable-device install-over plus process-restart migration smoke and framework-bound WorkManager/notification-service integration remain.
...
### 2026-08-14 — Version 39/40 to 41 migration audit

- Reviewed storage history at version 39 (`5c5b670`), the Room/DataStore migration (`0d694ef`), and version 40 (`7c676da`). Version 40 and 41 retain the same `comfer_settings` DataStore, `prefs_migrated_v2` flag, `comfer.db` Room database, schema version 1, entities, and DAOs.
- Raised release metadata to versionCode 41 / versionName 41.0.
- Added full v39 payload preflight before any destination mutation. Scalar types, exact app-list delimiter, folders, and exact widget schema are validated together; malformed folder/widget data leaves all legacy sources and the completion flag untouched.
- Fixed present-but-empty legacy app lists being skipped; they now migrate as `[]`. Batched all scalar imports into one DataStore transaction.
- Added execution-order/failure tests proving v39 read -> persist -> delete -> complete behavior and proving a completed v40 migration touches no legacy files. Unit tests, lint, and minified release assembly pass.
- Historical limitation: version 40 could already have swallowed malformed folder JSON and deleted its source. Version 41 cannot reconstruct data previously erased by version 40. Full evidence: `MIGRATION-VALIDATION-39-40-TO-41.md`.
